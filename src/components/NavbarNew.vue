<template>
    <nav class="navbar fixed-top navbar-expand-lg glass-nav px-4 py-2 mx-auto">
        <div class="container-fluid px-3">
            <!-- Brand -->
            <a class="navbar-brand gradient-logo fw-bold me-auto" href="#home">FARREL</a>
            ...
            <!-- Links -->
            <div class="collapse navbar-collapse" :class="{ show: isMenuOpen, 'menu-center': isMenuOpen }">
                <ul class="navbar-nav ms-lg-auto">
                    <li class="nav-item fade-link" v-for="(link, index) in links" :key="link.id"
                        :style="{ transitionDelay: isMenuOpen ? index * 0.1 + 's' : '0s' }">
                        <a class="nav-link" :class="{ active: activeSection === link.id }" :href="`#${link.id}`"
                            @click="closeMenu">{{ link.name }}</a>
                    </li>
                </ul>
            </div>
